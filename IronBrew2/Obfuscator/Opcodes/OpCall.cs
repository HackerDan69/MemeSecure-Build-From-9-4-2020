...
local Results = { Stk[A](Unpack(Stk, A + 1, Inst[D9_OP_B])) };
local Edx = 0;
for Idx = A, Inst[D9_OP_C] do 
	Edx = Edx + 1;
	Stk[Idx] = Results[Edx];
end
";
...
";
	...
	public class OpCallC0 : VOpcode
	{
		...
		public override string GetObfuscated(ObfuscationContext context) =>
			@"
local A = Inst[D9_OP_A]
local Results, Limit = _R(Stk[A](Unpack(Stk, A + 1, Inst[D9_OP_B])))
Top = Limit + A - 1
local Edx = 0;
for Idx = A, Top do 
	Edx = Edx + 1;
...
";
	...
	public class OpCallC1 : VOpcode
	{
		...
		public override string GetObfuscated(ObfuscationContext context) =>
			@"
local A = Inst[D9_OP_A]
Stk[A](Unpack(Stk, A + 1, Inst[D9_OP_B]))
";
		public override void Mutate(Instruction instruction)
		{
			instruction.B += instruction.A - 1;
...
";
	...
	public class OpCallC2 : VOpcode
	{
		...
		public override string GetObfuscated(ObfuscationContext context) =>
			@"
local A = Inst[D9_OP_A]
Stk[A] = Stk[A](Unpack(Stk, A + 1, Inst[D9_OP_B])) 
";
		public override void Mutate(Instruction instruction)
		{
			instruction.B += instruction.A - 1;
